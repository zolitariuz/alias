...
var Book = {
	...
	init: function() {
		...
		default_book_width	= 1100;
		default_book_height	= 715;
		default_page_width	= 1100;
		default_page_height	= 715;
	...
	zoom_auto: function() {
		...
		if (isiPhone()) {
			...
			if (( $(window).width() > 1100 ) && ( $(window).height() > 815 )) {
				$('#page').css({ width: 1100, height: 715 });
				$('#book').turn('size', 1100, 715);
			}
